...
const pam13ScoreConverter = (rawScores: SurveyScores): SurveyScores => {
    ...
    const minKey = 13;
    const maxKey = 52;
    if (rawScore < minKey) return { score: pam13ConversionTable[minKey] };
    if (rawScore > maxKey) return { score: pam13ConversionTable[maxKey] };

    return { score: pam13ConversionTable[rawScore] };
};
